...
export default function LaborForce() {
  const [data, setData] = useState<LabourForceData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await EconomicDataService.getLabourForce();
        setData(response.labour_force || []);

        // Extract unique countries from the data
        const countries = response.labour_force.reduce(
        ...
        setError("Failed to load labor force data");
        console.error(err);
      } finally {
        setLoading(false);
      }
              ...
              >
                {sortedData.length > 0 ? (
                  <LineChart
                    series={[
                      {
                        data: sortedData.map((item) => item.labour_force_total),
                        label: `Labor Force Total - ${
                          availableCountries.find(
                            (c) => c.code === selectedCountry
                          )?.name || "All Countries"
                        }`,
                        color: "hsl(29, 100.00%, 55.10%)",
                  ...
                  >
                    <Typography color="text.secondary">
                      No labor force data available for the selected filters
                    </Typography>
                  </Box>
                )}
              </Paper>
                    ...
                    <tbody>
                      {paginatedData.map((item) => (
                        <tr key={item.id}>
                          ...
                          >
                            {formatNumber(item.labour_force_total)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
