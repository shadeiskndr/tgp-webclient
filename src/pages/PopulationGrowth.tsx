...
export default function PopulationGrowth() {
  const [data, setData] = useState<PopulationGrowthData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await EconomicDataService.getPopulationGrowth();
        setData(response.population || []);

        // Extract unique countries from the data
        const countries = response.population.reduce(
        ...
        setError("Failed to load population growth data");
        console.error(err);
      } finally {
        setLoading(false);
      }
                ...
                {sortedData.length > 0 ? (
                  <LineChart
                    series={[
                      {
                        data: sortedData.map(
                          (item) => item.population_growth_rate
                        ),
                        label: `Population Growth Rate (Annual %) - ${
                          availableCountries.find(
                            (c) => c.code === selectedCountry
                          )?.name || "All Countries"
                        }`,
                        color: "hsl(271, 100.00%, 35.10%)",
                      },
                    ]}
                    xAxis={[
                      {
                  ...
                  >
                    <Typography color="text.secondary">
                      No population growth data available for the selected
                      filters
                    </Typography>
                  </Box>
                )}
                    ...
                    <tbody>
                      {paginatedData.map((item) => (
                        <tr key={item.id}>
                          ...
                          >
                            {item.population_growth_rate.toFixed(2)}%
                          </td>
                        </tr>
                      ))}
                    </tbody>
