...
export default function EducationExpenditure() {
  const [data, setData] = useState<EducationExpenditureData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await EconomicDataService.getEducationExpenditure();
        setData(response.education_expenditure || []);

        // Extract unique countries from the data
        const countries = response.education_expenditure.reduce(
        ...
        setError("Failed to load education expenditure data");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <Box>
      <Typography component="h1" variant="h4" sx={{ mb: 4, fontWeight: 700 }}>
        Education Expenditure (Total % of GDP) Analysis
      </Typography>
                ...
                {sortedData.length > 0 ? (
                  <LineChart
                    series={[
                      {
                        data: sortedData.map(
                          (item) => item.expenditure_percentage
                        ),
                        label: `Education Expenditure (Total % of GDP) - ${
                  ...
                  >
                    <Typography color="text.secondary">
                      No education expenditure data available for the selected
                      filters
                    </Typography>
                  </Box>
                )}
                    ...
                    <tbody>
                      {paginatedData.map((item) => (
                        <tr key={item.id}>
                          ...
                          >
                            {item.expenditure_percentage.toFixed(2)}%
                          </td>
                        </tr>
                      ))}
                    </tbody>
